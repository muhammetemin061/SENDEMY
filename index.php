...
<body>
	...
	font-weight:bolder;
	font-style:normal; font-size:7vh; color: #a200bb ">Popüler Kurslar !!</h2>
			</div>
		</div>
		<!-- Popular courses End -->
...
$stmt = $conn->prepare("SELECT v.id, v.user, v.baslik, v.aciklama, v.videookapak, v.fiyat FROM videolar v
                        ...
                        WHERE v.onay = ? AND s.id IS NULL");
$stmt->bind_param("ss", $user_id, $onay);
$stmt->execute();
$stmt->store_result();
...
while ($stmt->fetch()) {
	...
	if($_SESSION['oturum']=='acik'){?>
		<script>
			document.addEventListener("DOMContentLoaded", function() {
				// URL'de satinalim=success parametresi varsa alert göster
				const urlParams = new URLSearchParams(window.location.search);
				if (urlParams.has('satinalim') && urlParams.get('satinalim') === 'success') {
	...
	<script>
		document.addEventListener("DOMContentLoaded", function() {
			...
			const urlParams = new URLSearchParams(window.location.search);
			if (urlParams.has('satinalim') && urlParams.get('satinalim') === 'success') {
				alert("Satın almak için giriş yapmanız gerekmektedir");
			}
		});
	</script>
