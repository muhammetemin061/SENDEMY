...
<body>
	...
	font-weight:bolder;
	font-style:normal; font-size:7vh; color: #a200bb ">Satın Aldığım Kurslar!!</h2>
			</div>
		</div>
		<!-- Popular courses End -->
...
if(isset($_SESSION['userid'])) {
    ...
    // Kullanıcının satın aldığı kursları getirmek için sorgu
    $stmt = $conn->prepare("SELECT v.id, v.baslik, v.aciklama, v.video,v.videookapak
                           FROM videolar v
                           INNER JOIN satinalim s ON v.id = s.video_id
                           WHERE s.user_id = ?");
    $stmt->bind_param("i", $user_id);
    $stmt->execute();
    $result = $stmt->get_result();

    // Satın alınan kursların listesi
    while ($row = $result->fetch_assoc()) {
        echo '<form action="videolarımınbilgileri.php" method="GET">';
        echo '    <div class="box-wrap" itemprop="event" itemscope itemtype="http://schema.org/Course">';
        echo '        <div class="img-wrap" itemprop="image"><img src="' . $row['videookapak'] . '" alt="courses picture"></div>';
        echo '        <input type="submit" class="learn-desining-banner" name="' . $row['id'] . '" value="İzle">';
    ...
    // Kullanıcı oturum açmamışsa, uyarı mesajı gösterilebilir
